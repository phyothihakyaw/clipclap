...
It stores settings and an API key in extension storage on the user’s device, a save-folder handle in IndexedDB, and temporary clip state in session storage.
...
- Network calls for rewriting should go only to the configured model provider endpoint.
